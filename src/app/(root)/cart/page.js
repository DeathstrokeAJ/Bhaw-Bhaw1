...
//     const calculatedDiscount = (calculatedSubtotal * couponDiscount) / 100;
//     setDiscountAmount(calculatedDiscount);

//     setTotal(calculatedSubtotal - calculatedDiscount + deliveryFee);
...
const Cart = () => {
  ...
  const [validatingCoupon, setValidatingCoupon] = useState(false);
  const [error, setError] = useState("");
  const [deliveryFee] = useState(15); // constant delivery fee
  const [isPopupVisible, setIsPopupVisible] = useState(false);
  const [isPopupVisible1, setIsPopupVisible1] = useState(false);
  const [isPopupVisible2, setIsPopupVisible2] = useState(false);
  const dispatch = useDispatch();
